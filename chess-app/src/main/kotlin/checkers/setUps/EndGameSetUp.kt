...
class EndGameSetUp {
    fun setUp(): Board {
        return Board(8, 8, emptyMap(), emptySet(), emptyList())
            .putPieceAt(Position(2, 1), Piece("2 1", Color.WHITE, listOf(CheckersMoves(), MoveLimitation(1,1, CheckersCaptureMove())), PieceType.PAWN))
            .putPieceAt(Position(3, 2), Piece("3 2", Color.WHITE, listOf(CheckersMoves(), MoveLimitation(1,1, CheckersCaptureMove())), PieceType.PAWN))
